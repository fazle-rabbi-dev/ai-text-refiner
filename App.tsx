...
const App: React.FC = () => {
  ...
  const [inputLanguage, setInputLanguage] = useState<Language>(Language.ENGLISH);
  ...
  const handleRefine = useCallback(async () => {
    if (!inputText.trim()) {
      setApiError('Please enter some text to refine.');
      setOutputText('');
      return;
    }
    ...
    const result = await refineText(inputText, inputLanguage, selectedTone);
    ...
    setIsLoading(false);
  }, [inputText, inputLanguage, selectedTone]);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans flex flex-col items-center p-4 sm:p-6 lg:p-8">
      <header className="w-full max-w-4xl text-center mb-8">
        ...
        <p className="text-slate-400 mt-2">Instantly rewrite your text into different styles.</p>
      </header>
      ...
      <main className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-slate-800/50 rounded-xl shadow-lg border border-slate-700 p-6">
            <h2 className="text-lg font-semibold text-slate-300 mb-3">Input Language</h2>
          ...
          <div className="bg-slate-800/50 rounded-xl shadow-lg border border-slate-700 p-6">
             <label htmlFor="inputText" className="text-lg font-semibold text-slate-300 mb-3 block">Your Text</label>
            <textarea
              id="inputText"
              value={inputText}
        ...
        <div className="bg-slate-800/50 rounded-xl shadow-lg border border-slate-700 p-6 relative min-h-[300px] lg:min-h-full flex flex-col">
           <div className="flex justify-between items-center mb-3">
              <h2 className="text-lg font-semibold text-slate-300">Refined Output</h2>
              {outputText && !isLoading && (
                ...
                >
                  <CopyIcon copied={isCopied} className="w-4 h-4" />
